...
def inference(hdr_data: np.ndarray, patch_size: int,model: HybridCNN): 
    ...
    with torch.no_grad():
        for patch in tqdm(dataloader):
            ...
            condition = (label_pred <= 0.85).all(dim=1)
            labels[condition] = 0
...
hdr_path = "D:\\Machine Learning\\Texmin_HSI\\HSI_Classification_Updated\\Minerals_Dataset\\HDR_Data\\Extracted_Folder\\0025\\A.hdr.h5"
...
# ax[0].imshow(hdr_data[ch,:,:])
# ax[1].imshow(hsi[ch,:,:])
# plt.show()

mask_act_path = "D:\\Machine Learning\\Texmin_HSI\\HSI_Classification_Updated\\Minerals_Dataset\\Minerals_mat_files\\0025_A_gt.mat"
mask_actual = loadmat(mask_act_path)['MASK']
model_path = "D:\\Machine Learning\\Texmin_HSI\\HSI_Classification_Updated\\HybridSN\\HSI_Minerals\\hybrid_cnn_model4.pth"
