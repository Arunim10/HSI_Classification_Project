...
hdr_files = list(glob("D://Machine Learning//Texmin_HSI//HSI_Classification_Updated//Minerals_Dataset//HDR_Data//Extracted_Folder//*//*.hdr.h5"))
...
class InteractiveWavelengthSlider:
    ...
    def update_wavelength(self, val):
        ...
        self.im.set_data(self.hsi_data[:, :, self.current_wavelength_index])
        self.ax_image.set_title(f'Wavelength: {self.wavelengths[self.current_wavelength_index]:.2f} nm')
        print("New Wavelength ",self.wavelengths[self.current_wavelength_index])
        self.wavelength_indicator.set_xdata([self.wavelengths[self.current_wavelength_index]])
        self.fig.canvas.draw_idle()
...
hsi = np.transpose(loadmat(df[df['ID']==id].iloc[0]['HSI_Path'])['HDR'],axes=(1,2,0))
# print(df.head())

# print(hsi.shape)
# hsi1 = np.transpose(loadmat(df[df['ID']==id].iloc[0]['HSI_Path'])['HDR'],axes=(1,2,0))
# hsi2 = np.transpose(loadmat(df[df['ID']==id].iloc[1]['HSI_Path'])['HDR'],axes=(1,2,0))
# hsi = hsi1-hsi2
# print(df.loc[id,'Mineral'])
# print(df[df['ID']==id].iloc[1]['Mineral'])
# mask = loadmat(df.loc[id,'MASK_Path'])['MASK']
# hsi = loadmat(df.loc[id,'HSI_Path'])['HDR'] # (256,320,410)
...
# random_value = 1
# # print(random_value)
# # hsi_red_mod = np.transpose(np.array([hsi_red[:,0],hsi_red[:,1]* random_value,hsi_red[:,2]]),axes=(1,0))
# # print(hsi_red_mod.shape)
# reconstructed_data = pca.inverse_transform(hsi_red).reshape(hsi.shape[1],hsi.shape[2],-1)
# print(reconstructed_data.shape)
# # fig,ax = plt.subplots(1,2)
# # ax[0].imshow(hsi[0,:,:])
# # ax[1].imshow(reconstructed_data[:,:,0])
# tf = A.Compose([
...
# print(hsi_ori[125,180,:])
# print(reconstructed_data[125,180,:])
slider = InteractiveWavelengthSlider(hsi, np.array(wavelengths))
# slider = InteractiveWavelengthSlider(hsi, np.array(wavelengths))
slider.show()
