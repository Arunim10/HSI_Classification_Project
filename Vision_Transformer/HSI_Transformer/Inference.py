...
def inference(hdr_data: np.ndarray, patch_size: int,model: MViT): 
    ...
    with torch.no_grad():
        for patch in tqdm(dataloader):
            ...
            # condition = (label_pred <= 0.95).all(dim=1)
            # labels[condition] = 0
...
hdr_path = "D:\\Machine Learning\\Texmin_HSI\\HSI_Classification_Updated\\Minerals_Dataset\\HDR_Data\\Extracted_Folder\\0001\\A.hdr.h5"
...
patch_size = 15
mask_act_path = "D:\\Machine Learning\\Texmin_HSI\\HSI_Classification_Updated\\Minerals_Dataset\\Minerals_mat_files\\0001_A_gt.mat"
mask_actual = loadmat(mask_act_path)['MASK']
model_path = "D:\\Machine Learning\\Texmin_HSI\\HSI_Classification_Updated\\Vision_Transformer\\HSI_Transformer\\MViT_Model_1.pth"
model = MViT(patch_size,76,3,512,3,8,0.1,256,256,"gelu")
